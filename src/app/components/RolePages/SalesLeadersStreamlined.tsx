    ...
    <section className="relative isolate overflow-hidden py-16 sm:py-24 bg-white">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl text-center mx-auto">
          <p className="text-2xl font-semibold uppercase tracking-[0.35em] text-lime-400">Sales Flow</p>
          <h2 className="mt-3 font-extrabold text-[#0a2440] text-5xl">
          ...
          {steps.map(({ icon: Icon, title, description }, idx) => (
            <li key={idx} className="grid grid-cols-[64px_auto] items-start gap-5">
              ...
              <div className="relative flex h-16 w-16 items-center justify-center rounded-xl bg-[#0a2440]/5 ring-1 ring-[#0a2440]/10 shadow-sm">
                <Icon className="h-7 w-7 text-[#0a2440]" aria-hidden />
                {/* simple notch accent */}
                <div className="absolute left-0 top-1/2 -translate-y-1/2 h-3 w-1 rounded-r bg-[#0a2440]" aria-hidden />
                {/* vertical connector */}
                {idx < steps.length - 1 && (
              ...
              <div className="rounded-xl border border-[#0a2440]/10 bg-white p-4 sm:p-5 shadow-[0_10px_30px_-20px_rgba(10,36,64,0.25)] transition-shadow hover:shadow-[0_20px_45px_-25px_rgba(10,36,64,0.3)]">
                <span className="mb-1 inline-block text-xs font-semibold uppercase tracking-wide text-[#0a2440]">Step {idx + 1}</span>
                <h3 className="text-lg font-semibold text-[#0a2440] sm:text-xl">{title}</h3>
                <p className="mt-2 text-lg leading-relaxed text-[#0a2440] sm:text-lg">{description}</p>
              </div>
            </li>
          ))}
        </ol>
